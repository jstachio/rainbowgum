...
### A wrong "fix", then reverted - `useGetBytes` is the correct mechanism instead

**This subsection originally reported a "fix" that was later found to be an
architectural mistake and reverted - kept below (struck through) for the record, then
corrected.**
...
`encodeToBuffer(...)` ... [+16.2% throughput, +2.8%→-6.1% RSS swing]~~

The mistake: that change made `StringBuilderBuffer` (the buffer backing
`WriteMethod.STRING`) secretly convert to bytes and write through the `byte[]` overload,
so it stopped calling `LogOutput.write(LogEvent, String)` at all - breaking
`WriteMethod.STRING`'s actual contract. Adam's objection: "We should not be calling the
write that takes bytes if STRING is requested. There are outputs that do not want any
byte conversion. A JFR encoder comes to mind." The in-lock `getBytes()` timing was a
real, legitimate thing to want to fix - but the fix belonged in a new, separate
mechanism (`LogEncoder.Builder#useGetBytes(true)`, already added to `core` for
unrelated reasons, paired with the *default* `BYTES`-hinting output), not in mutating
`STRING`'s own semantics. Reverted in `core` (`StringBuilderBuffer` is back to
genuinely calling `write(LogEvent, String)`, same as the original "Caveat" paragraph
above already described correctly) and in this benchmark module
(`StringStdOutOutput`/`OUTPUT_TYPE=STRING` is back to its original behavior).

Added a second, correctly-scoped experiment instead: a new `get-bytes-ttll:///` encoder
scheme (`GetBytesEncoderConfigurator`, `ENCODER_TYPE=GET_BYTES`) that builds the
standard TTLL encoder with `useGetBytes(true)`, paired with the **default** stdout
output (`BYTES`-hinting, unchanged) - the `getBytes()` conversion happens in
`StringBuilderBufferBytes.encodeToBuffer(...)`, outside the lock, same timing
`DirectByteBufferBuffer`'s `CharsetEncoder` work already gets, with no change to what
overload the output receives. Retested all three (TTLL, GraalVM 25.3.4, default
appender type, `/greet/world`, 3 runs each, same sandbox session, single-pass - not
reversed-order-confirmed this round):

| | default (`BYTES`/`CharsetEncoder`) | `OUTPUT_TYPE=STRING` (reverted) | `ENCODER_TYPE=GET_BYTES` |
|---|---:|---:|---:|
| throughput (avg of 3) | 41,268 req/s | 40,858 req/s | **42,729 req/s** |
| p50 latency | 1.12 ms | 1.14 ms | **1.10 ms** |
| RSS (avg) | 70.6 MB | 46.7 MB | 48.0 MB |

`OUTPUT_TYPE=STRING`, now that it's back to genuinely calling
`write(LogEvent, String)` (in-lock `getBytes()`, same as Logback's own shape), is
**roughly flat** against default here (-1.0%) - it does **not** reproduce this file's
much earlier +9.6% measurement for the same configuration (see the "Caveat" section
above); consistent with this whole file's standing methodology note, absolute numbers
and even signs of small deltas are not reliable across separate sandbox sessions, only
a same-batch comparison is. `ENCODER_TYPE=GET_BYTES` - the correctly-scoped
outside-the-lock `getBytes()` mechanism - **is** a real, positive, same-batch win
(+3.5% throughput, -1.8% p50) over both default and `STRING`, which is the directionally
sensible result: it gets `getBytes()`'s fast path *and* keeps it outside the lock,
without `STRING`'s in-lock cost. RSS is a separate story this round - both `STRING` and
`GET_BYTES` show a large RSS drop against default (-34%/-32%) that tracks with which
output/encoder is active more than with the getBytes-vs-CharsetEncoder question itself;
...
**Correction**: this section originally also claimed the `getBytes()`-based `STRING`
path above "already closes that gap entirely and then some on its own" (30,034 req/s).
That number came from the architectural mistake described and reverted in the
"A wrong 'fix', then reverted" section above - `OUTPUT_TYPE=STRING` no longer produces
anything close to that number (see the corrected table there); the correctly-scoped
replacement, `ENCODER_TYPE=GET_BYTES`, was a smaller, real +3.5% win over default in its
own same-batch comparison, not directly compared against Log4j2 in that batch. Stacking
`ENCODER_TYPE=GET_BYTES` + `SYNCHRONIZED_THREAD_LOCAL_BUFFER` together, then comparing
against Log4j2 in one controlled batch, is the accurate version of the "obvious next
experiment" this section originally pointed at.

## The gap closes: `CharBuffer.wrap` → `getChars` copy encoding

Everything above this section was measured against an older core. Adam moved to a
second agent (codex, for credits) to run an *isolated*, no-HTTP, no-Rainbow-Gum-appender
encoding-only microbenchmark (`feature/encoding-benchmark`) - and found the real issue
himself, not the agent: `DirectByteBufferBuffer.encodeToByteBuffer()`'s
`CharBuffer.wrap(stringBuilder)` was measurably slower than copying the characters out
first via `StringBuilder.getChars(...)` into a real, reused, array-backed `CharBuffer`.
`CharBuffer.wrap(CharSequence)` wraps `StringBuilder` as a view - the `CharsetEncoder`
then has to call `charAt(i)` per character through virtual dispatch, one call per
character, instead of bulk-copying a real backing array. Landed as
`feature/log-encoder-copy-chars` (a new, reused `CharBuffer` field, filled via
`getChars`, pre-sized alongside `stringBuilder`/`byteBuffer` the same way this file's
own earlier sizing fix already did for the other two stores) - this benchmark's own
branch was rebased onto it (`git rebase --onto`, one real conflict, in exactly the
`DirectByteBufferBuffer` constructor both branches had independently touched - see this
repo's commit history for the resolution). Separately, this same session's isolated
flame-graph work (scratchpad, not committed - see the earlier findings summarized just
above) had already confirmed two things worth restating here: Log4j2's real advantage
involves a buffering design where one thread's flush can end up writing bytes it did
not itself produce (measured harmless for correctness, but rejected on principle -
Rainbow Gum's own attempt at the same trick, `SYNCHRONIZED_SHARED_BUFFER`/
`SYNCHRONIZED_DEFERRED_FLUSH`, cost more than it ever gained and was kept only as a
documented negative result); and profiling alone did not close the gap on its own.

The hope going into this retest: does the `getChars` copy-encoding fix, combined with
the already-established `SYNCHRONIZED_THREAD_LOCAL_BUFFER` win (see "Closing the gap"
above), get close enough to Log4j2 *without* giving up the one-event-one-owner
guarantee that buffering requires trading away? Retested exactly that configuration
(`APPENDER_TYPE=SYNCHRONIZED_THREAD_LOCAL_BUFFER`, default output, TTLL,
`/greet/world`), both native-image (GraalVM 25.3.4) and, at Adam's request, plain
HotSpot (Temurin 26.0.2) - 6 runs each (3 forward + 3 reversed start order):

| | Rainbow Gum (`SYNCHRONIZED_THREAD_LOCAL_BUFFER`) | Log4j2 (default) | gap |
|---|---:|---:|---:|
| native-image throughput | 32,373 req/s | 32,291 req/s | RG **+0.3%** |
| native-image RSS avg | 60.1 MB | 63.3 MB | RG -5.0% |
| HotSpot throughput | 23,689 req/s | 22,407 req/s | RG **+5.7%** |
| HotSpot RSS avg | 574.8 MB | 625.1 MB | RG -8.0% |

**The gap is gone under native-image** (Rainbow Gum is a fraction of a percent *ahead*,
well within this benchmark's own noise floor - call it a tie, not a win) **and Rainbow
Gum leads outright on HotSpot**, both without touching the locking-strategy
compromises explored and rejected in this session's isolated profiling work. This is
the encoding-layer fix (a straightforward, uncontroversial bulk-copy-instead-of-virtual-
dispatch performance fix, nothing architecturally unusual) plus the already-established
`SYNCHRONIZED_THREAD_LOCAL_BUFFER` locking win - not a new architectural trick, just
removing real, avoidable overhead from the path every configuration in this file was
already using. Absolute numbers here are, as always in this file, not directly
comparable to earlier sections' numbers from this same sandbox (see the methodology
note at the top) - only the paired RG-vs-Log4j2 delta measured in this identical batch
is meaningful, and that delta is now close to zero under native-image and clearly
favorable under HotSpot.

**Not yet retested with this core**: every other scenario in this file (structured
logging, non-Latin1 content, `LOCK_NEW_BUFFER`, the `LOG_LEVEL=ERROR` baseline, Rainbow
Gum's own default `LOCK_THREAD_LOCAL_BUFFER` type head-to-head against Log4j2) predates
this fix and has not been re-run against it.
...
* `SYNCHRONIZED_THREAD_LOCAL_BUFFER` + `ENCODER_TYPE=GET_BYTES` stacked together, then
  compared against Log4j2 in one controlled batch - the accurate version of what this
  bullet originally proposed with `OUTPUT_TYPE=STRING` before that path's numbers were
  found to rest on a reverted mistake (see "A wrong 'fix', then reverted" above). Also
  never tried on plain HotSpot (only GraalVM 21 and 25.3.4 so far).
