 ...
 *
 * <pre>
 * --url &lt;uri&gt;          default http://localhost:8080/greet/world
 ...
 * --threads &lt;kind&gt;     "virtual" (default) or "platform" - client-side dispatch
 * </pre>
 */
record Args(URI url, int concurrency, int warmupSeconds, int durationSeconds, long pid, String label,
		@Nullable Path out, boolean platformThreads) {
	...
	static Args parse(String[] args) {
		...
		String urlStr = flags.getOrDefault("url", "http://localhost:8080/greet/world");
		...
		@Nullable String outStr = flags.get("out");
		@Nullable Path out = outStr == null ? null : Path.of(outStr);
		boolean platformThreads = "platform".equals(flags.get("threads"));
		return new Args(URI.create(urlStr), concurrency, warmup, duration, pid, label, out, platformThreads);
	}
