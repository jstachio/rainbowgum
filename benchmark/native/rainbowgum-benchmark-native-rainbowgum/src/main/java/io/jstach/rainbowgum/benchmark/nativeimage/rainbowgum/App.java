 ...
 * {@code LogOutput.WriteMethod#STRING} instead of {@code BYTES} - this only changes which
 * {@code LogOutput.write(...)} overload is called (the output genuinely receives a
 * {@link String}, encoded lazily inside {@code LogOutput#write(LogEvent, String)}'s own
 * default {@code getBytes(UTF_8)} implementation). {@code OUTPUT_TYPE=BUFFERED} swaps it
 * for {@link BufferedStdOutOutput}, a raw-FD output wrapped in a plain
 * {@code java.io.BufferedOutputStream} instead of {@link System#out}'s auto-flushing
 * {@code PrintStream} - meant to be paired with
